...
function doDeleteAllMacros (input) {
  var getTickets = {
    url: input,
    type: 'DELETE',
    dataType : "json",
                    ...
                    for (var i=0; i<viewSelectList.length; i++) {
                      ...
                      if (!viewExist) {
                        ...
                        (function(counterI){
                                  ...
                                  function(ticketField){
                                    for (var cfd=0; cfd<ticketFieldDest.ticket_fields.length; cfd++) {
                                      if (ticketField.ticket_field.title == ticketFieldDest.ticket_fields[cfd].title) {
                                        ...
                                        // for (var col=0; col<viewSelectList[counterI].execution.fields.length; col++) {
                                        //   if (viewSelectList[counterI].execution.fields[col].title == ticketFieldDest.ticket_fields[cfd].title) {
                                        //     viewSelectList[counterI].execution.fields[col].id = ticketFieldDest.ticket_fields[cfd].id;
                                        //     viewSelectList[counterI].execution.fields[col].url = ticketFieldDest.ticket_fields[cfd].url;
                                        //   }
                                        // }
                                        checkQueViewSelectList (counterArray, viewSelectList, counterI);
                                      }
                          ...
                          if (viewSelectList[i].execution.group === null) {
                            /* DO NOTHING */
                          } else {
                          ...
                          if (macrosSelectList[i].actions.length > 0) {
                            ...
                            for (var a=0; a<macrosSelectList[i].actions.length; a++) {
                              (function(counterA){
                                    ...
                                    function(brands){
                                      ...
                                      for (var br=0; br<brandsDest.brands.length; br++) {
                                        if (brands.brand.name == brandsDest.brands[br].name) {
                                          ...
                                          console.log('brands found');
                                          actionCounter++;
                                          macrosSelectList[counterI].actions[counterA].value = brandsDest.brands[br].id;
                                          if (actionCounter == macrosSelectList[counterI].actions.length) {
                                            processCounter++;
                                            if (processCounter == 2) {
                                              doCreateMacros(macrosSelectList[counterI]);
                                ...
                                } else if (macrosSelectList[i].actions[a].field == 'ticket_form_id') {
                                  client.request(getTicketFormsById(macrosSelectList[i].actions[a].value)).then(
                                    function(tfForm){
                                      var brandsIsFound = false;
                                      for (var br=0; br<ticketFormsDest.ticket_forms.length; br++) {
                                        if (tfForm.ticket_form.name == ticketFormsDest.ticket_forms[br].name) {
                                          brandsIsFound = true;
                                          console.log('ticket form found found');
                                          actionCounter++;
                                          macrosSelectList[counterI].actions[counterA].value = ticketFormsDest.ticket_forms[br].id;
                                          if (actionCounter == macrosSelectList[counterI].actions.length) {
                                            processCounter++;
                                            if (processCounter == 2) {
                                              doCreateMacros(macrosSelectList[counterI]);
                                      ...
                                      console.log('===== ticket not found =====');
                                        updateProgress('Macros', '<b>' + macrosSelectList[counterI].title + '</b> Error. Ticket forms not exist: ' + tfForm.ticket_form.name);
                                      }
                                    },
                                    function(tfFormError){
                                      console.log('===== tfFormError =====');
                                      console.log(tfFormError);
                                    });
                                } else {
                                  actionCounter++;
...
function checkQueViewSelectList (counterArray, selectList, counterI) {
  ...
  if (selectList[counterI].execution.group === null) {
    /* DO NOTHING */
  } else {
  ...
  client.request(getMacros()).then(
    function (data){
      ...
      for (var i=0; i<data.macros.length; i++) {
        mCount++;
        if (mCount == 99) {
          ids += data.macros[i].id;
          mCount = 0;
          console.log(ids)
          url = '/api/v2/macros/destroy_many.json?ids=' + ids
          client.request(doDeleteAllMacros(url)).then(
            function (deleteData){
              console.log(deleteData);
            },
            function(errorDeleteData){
